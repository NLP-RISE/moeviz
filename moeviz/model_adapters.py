...
class ModelAdapter:
    ...
    def get_router_hook(
        self, layer_id: int, token_queue: Any, routing_queue: Any, tokenizer: Any = None
    ) -> Callable:
    ...
    def register_hooks(
        ...
        token_hook_handle = model.model.embed_tokens.register_forward_pre_hook(
            token_hook
        )
        ...
        router_hook = self.get_router_hook(
            layer_id, token_queue, routing_queue, tokenizer
        )
...
class MixtralAdapter(ModelAdapter):
    ...
    def get_router_hook(
        ...
        def hook(module, input, output):
            ...
            selected_experts = self.process_router_logits(
                router_logits.clone().detach()
            )
            tokens = token_queue.get()
